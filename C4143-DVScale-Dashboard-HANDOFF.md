...
| 初版日期 | 2026-07-28 |
| 最後更新 | 2026-08-28 |
| 最新腳本 | `C4143-CRD4.1-Dashboard.user.js`（v1.11.2） |
...
| 檔案 / Key | 說明 |
| --- | --- |
| `C4143-CRD4.1-Dashboard.user.js` | 最新主交付物與固定安裝入口，Tampermonkey userscript v1.10.0 |
...
載體頁面選用：`https://azurecsi.visualstudio.com/_apis/projects?api-version=6.0#crdv41qual`（回傳很小的 JSON，載入快、沒有 ADO SPA 的干擾）。腳本只在 `location.hash` 含 `dvdash` 時啟動，因此不會影響正常瀏覽 ADO。
...
2. 把 `C4143-CRD4.1-Dashboard.user.js` 拖進 Chrome 視窗（或 Tampermonkey → Utilities → Import from file），按 Install。
3. 加入書籤：`https://azurecsi.visualstudio.com/_apis/projects?api-version=6.0#crdv41qual`
...
| 啟動網址 | `https://azurecsi.visualstudio.com/_apis/projects?api-version=6.0#crdv41qual` |
...
1. 舊版本檔名必須最後一次手動改裝 `C4143-CRD4.1-Dashboard.user.js`；之後 Tampermonkey 可依 GitHub 固定網址與 `@version` 自動更新。
...
- 最新 userscript：`C4143-CRD4.1-Dashboard.user.js`（目前 `@version` 1.8.6）
...
`C4143-CRD4.1-Dashboard.user.js`

固定 Raw URL：

`https://raw.githubusercontent.com/brianlin-19780816/ADO-Test-state-monitoring-C4143-CRDv4.1-Qual/main/C4143-CRD4.1-Dashboard.user.js`
...
1. 所有正式修改都直接更新固定檔名 `C4143-CRD4.1-Dashboard.user.js`。
